...
if os.path.isfile('generation_ts_1.log'):
    os.remove('generation_ts_1.log')
    with open('generation_ts_1.log', 'w') as log_:
...
with open('generation_ts_1.log', 'a') as log_:
...
while 1:
    ...
    for i in range(20,99):
        try:
            ...
            filename='generation_ts_1.'+str(time.time())+'.data'

            agent_data = Element('agent_data',{'description':'','group':'','os_name':'arduino','interval':interval,'version':version,'timestamp':timestamp,'agent_name':agent_name,'timezone_offset':timezone_offset, 'address':address})
